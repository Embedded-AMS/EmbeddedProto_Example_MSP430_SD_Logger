...
int wait_ready (void)     
{
    ...
    for (tmr = 5000; tmr; tmr--) {     
        rcvr_mmc(&d, 1);
        if (d == 0xFF) return 1;
        uint8_t i = (sdInterface ->freq/1000000);for(;i>0; i--){__delay_cycles(100);};
    }
        ...
        if (send_cmd((8), 0x1AA) == 1) {     
            ...
            if (buf[2] == 0x01 && buf[3] == 0xAA) {         
                for (tmr = 1000; tmr; tmr--) {             
                    ...
                    uint8_t i = (sdInterface ->freq/1000000);for(;i>0; i--){__delay_cycles(1000);};
                }
...
uint8_t validateCSD(void)
{
  ...
  if ((send_cmd((9), 0) == 0) && rcvr_datablock(csd0, 16))
    if ((send_cmd((9), 0) == 0) && rcvr_datablock(csd1, 16))
    {
