...
void SDCard_init(void)
{
    ...
    param.selectClockSource = (0x0080);
    param.clockSourceFrequency = 8000000;
    param.desiredSpiClock = 400000;
    param.msbFirst = (0x2000);
    param.clockPhase = 0x00;
    param.clockPolarity = (0x4000);
    param.spiMode = (0x0000);
    EUSCI_B_SPI_initMaster(0x0640, &param);

    
    EUSCI_B_SPI_enable(0x0640);

    
    EUSCI_B_SPI_clearInterrupt(0x0640, (0x0001));
}
...
void SDCard_fastMode(void)
{
	...
	EUSCI_B_SPI_changeMasterClock(0x0640, &param);
}
...
{
    uint16_t gie = __get_SR_register() & (0x0008);              
    ...
    while (size--){
    	...
    	while(!EUSCI_B_SPI_getInterruptStatus(0x0640, (0x0002)));
    	
    	EUSCI_B_SPI_transmitData(0x0640, 0xff);
    	
    	while(!EUSCI_B_SPI_getInterruptStatus(0x0640, (0x0001)));
    	*pBuffer++ = EUSCI_B_SPI_receiveData(0x0640);
    }
...
void SDCard_sendFrame(uint8_t *pBuffer, uint16_t size)
{
    ...
    while (size--){
    	...
    	EUSCI_B_SPI_transmitData(0x0640, *pBuffer++);
    }
    
    while(EUSCI_B_SPI_isBusy(0x0640));

    
    
    EUSCI_B_SPI_receiveData(0x0640);
...
SDCardLib_Status SDCard_detectCard(void)
{
    ...
    GPIO_setAsInputPinWithPullUpResistor(7, (0x0004));

	if (GPIO_getInputPinValue(7, (0x0004))) {
...
{
	return RTC_C_getCalendarTime(0x04A0);
}

void SDCard_setRTCTime(Calendar *curTime)
{
    
    RTC_C_initCalendar(0x04A0,
    		curTime,
			(!((0x0080))));

    
    RTC_C_startClock(0x04A0);
}
